...
create_registry_input_file <-
  function(filename = 'cancerRADAR_input.xlsx', verbose = TRUE) {
    ageg.list <- c(
      'total',
      paste0(
        sprintf('%02d', seq(0, 80, 5)),
        '_',
        sprintf('%02d', seq(4, 84, 5))
      ),
      '85'
    )
    ...
    dat.list[['data_info']] <-
      dplyr::tribble(
        ...
        paste0(
          'Period over which this data was collected (2018-2022, 2013-2017, 2008-2012, 2003-2007):'
        ),
        paste0(
          'Data source of birth-country in registry (e.g. statistical Agency, hospital data, ...):'
        ),
        paste0(
          'Data source of population-at-risk (e.g. statistical Agency, national survey data, ...):'
        )
      )
          ...
          dat.list[[paste(item_, sex_, sep = '_')]] <-
            cbind(
              ...
              paste(
                item_,
                sex_,
                if (item_ == 'py') ageg.list else c(ageg.list, qual.list),
                sep = '_'
              ) |>
    ...
    if (!file.exists(filename)) {
      ...
      readme.file <- system.file(
        "extdata",
        "readme_cancerradarr_input_file.xlsx",
      ...
      for (sn_ in names(dat.list)) {
        ...
        if (sn_ == 'data_info') {
          ...
          addStyle(
            wb,
            sn_,
            createStyle(locked = FALSE, border = 'TopBottomLeftRight'),
            cols = 2,
            rows = 2:6
          )
          suppressWarnings(dataValidation(
            wb,
            sn_,
            cols = 2,
            rows = 2,
            type = "list",
            value = "'py_male'!$A$2:$A$252"
          ))
        } else {
      ...
      if (verbose) {
        cat(
          '\n>',
          filename,
          'has been created, please fill it with cancer registry data'
        )
      }
    } else {
      warning(
        paste0(
          filename,
          ' already exists on your hard drive. Nothing has been done. ',
          'If you are sure to want to overwrite this file please type unlink("',
          filename,
          '") ',
