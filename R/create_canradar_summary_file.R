...
#' @param py.min integer, the minimum number of person-years per age group to be considered during the aggregation step (default is 100)
...
create_canradar_summary_file <-
  function(
    ...
    data.info <- openxlsx::read.xlsx(filename.in, 'data_info') |>
      magrittr::set_colnames(c('inf.lab', 'inf.val'))
    ...
    dat.in.long.raw <-
      left_join(
        dat.in.can,
        dat.in.pop,
        by = c(common.vars, 'sex', 'ageg'),
        multiple = "all"
      ) |>
      mutate(ncan = round(.data$ncan), py = round(.data$py)) |>
      left_join(dat.aggr, by = 'cob_iso3') |>
      left_join(
        dat.asr.cat |> select(-c('asr')),
        by = c('cob_iso3', 'sex', 'can')
      ) |>
      mutate(
        across(
          any_of(c(
            setdiff(colnames(dat.aggr), 'any_migr'),
            colnames(dat.asr.cat)
          )),
          ~ replace(.x, is.na(.x), 'Other')
        ),
    ...
    dat.ref.in.pop <- dat.in.pop |>
      filter(.data$cob_iso3 == ref.cob.iso3) |>
      rename('pyref' = 'py') |>
      select(-c('cob_iso3'))
    dat.ref.in.can <- dat.in.can |>
      filter(.data$cob_iso3 == ref.cob.iso3) |>
      rename('ncanref' = 'ncan') |>
      select(-c('cob_iso3'))
    dat.ref.in.qual <- dat.in.qual |>
      filter(.data$cob_iso3 == ref.cob.iso3) |>
      select(-c('cob_iso3'))
    ...
    aggr.level.list <- c(
      if (include.by.cob.stat) 'cob_iso3',
      'un_region',
      'un_subregion',
      'hdi_cat',
      'any_migr',
    ...
    for (aggr.level in aggr.level.list) {
      ...
      dat.ir <- compute_incidence_rates(
        dat.in.long,
        aggr.level = aggr.level,
        ncan.lab = 'ncan',
        py.lab = 'py',
        ncan.min = ncan.min,
        py.min = py.min
      )
      dat.ir.aggr <- compute_incidence_rates(
        dat.in.long.aggr,
        aggr.level = aggr.level,
        ncan.lab = 'ncan',
        py.lab = 'py',
      ...
      dat.ref.ir <- compute_incidence_rates(
        dat.ref.in.long,
        aggr.level = NULL,
        ncan.lab = 'ncanref',
        py.lab = 'pyref',
      ...
      dat.ref.ir.aggr <- compute_incidence_rates(
        dat.ref.in.long.aggr,
        aggr.level = NULL,
        ncan.lab = 'ncanref',
        py.lab = 'pyref',
      ...
      dat.ref.globocan.ir <- compute_incidence_rates(
        dat.ref.in.long.globocan,
        aggr.level = NULL,
        ncan.lab = 'ncanref',
        py.lab = 'pyref',
      ...
      dat.ref.globocan.eu27.ir <- compute_incidence_rates(
        dat.ref.in.long.globocan.eu27,
        aggr.level = NULL,
        ncan.lab = 'ncanref',
        py.lab = 'pyref',
      ...
      dat.ref.globocan.euUN.ir <- compute_incidence_rates(
        dat.ref.in.long.globocan.euUN,
        aggr.level = NULL,
        ncan.lab = 'ncanref',
        py.lab = 'pyref',
      ...
      if (aggr.level == 'asr_rank_cat') {
        ## special case of burden aggregation
        dat.pr <- compute_proportional_rates_burden(
          dat.in.long,
          dat.in.long.raw,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
        ...
        dat.pr.aggr <- compute_proportional_rates_burden(
          dat.in.long.aggr,
          dat.in.long.raw,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
        ...
        dat.pr <- compute_proportional_rates(
          dat.in.long,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
          ncan.min = ncan.min
        )
        dat.pr.aggr <- compute_proportional_rates(
          dat.in.long.aggr,
          dat.in.long,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
      ...
      dat.ref.pr <- compute_proportional_rates(
        dat.ref.in.long,
        aggr.level = NULL,
        ncan.lab = 'ncanref',
        ncan.min = ncan.min
      )
      dat.ref.pr.aggr <- compute_proportional_rates(
        dat.ref.in.long.aggr,
        dat.ref.in.long,
        aggr.level = NULL,
        ncan.lab = 'ncanref',
      ...
      dat.cir <- compute_crude_incidence_rates(
        dat.in.long,
        aggr.level = aggr.level,
        ncan.lab = 'ncan',
        py.lab = 'py',
      ...
      dat.ref.cir <- compute_crude_incidence_rates(
        dat.ref.in.long,
        aggr.level = NULL,
        ncan.lab = 'ncanref',
        py.lab = 'pyref',
      ...
      dat.asir <- compute_age_standardized_incidence_rates(
        dat.in.long,
        std.pop,
        aggr.level = aggr.level,
        ncan.lab = 'ncan',
      ...
      dat.ref.asir <- compute_age_standardized_incidence_rates(
        dat.ref.in.long,
        std.pop,
        aggr.level = NULL,
        ncan.lab = 'ncanref',
          ...
          left_join(
            ...
            reframe(
              incidence_rates_ratio(
                ncan = .data[[ncan.lab]],
                py = .data[[py.lab]],
                ncanref = .data[[ncanref.lab]],
                pyref = .data[[pyref.lab]],
                ncan.min = ncan.min,
                py.min = py.min
              )
            )
      ...
      dat.cirr <- compute_incidence_rates_ratio(
        dat.in.long,
        dat.ref.in.long,
        aggr.level = aggr.level,
        ncan.min = ncan.min,
      ...
      dat.globocan.cirr <- compute_incidence_rates_ratio(
        dat.in.long,
        dat.ref.in.long.globocan,
        aggr.level = aggr.level,
        ncan.min = ncan.min,
      ...
      dat.globocan.eu27.cirr <- compute_incidence_rates_ratio(
        dat.in.long,
        dat.ref.in.long.globocan.eu27,
        aggr.level = aggr.level,
        ncan.min = ncan.min,
      ...
      dat.globocan.euUN.cirr <- compute_incidence_rates_ratio(
        dat.in.long,
        dat.ref.in.long.globocan.euUN,
        aggr.level = aggr.level,
        ncan.min = ncan.min,
      ...
      dat.asirr <- compute_age_standardized_incidence_rate_ratio(
        dat.in.long,
        dat.ref.in.long,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.asirr <- compute_age_standardized_incidence_rate_ratio(
        dat.in.long,
        dat.ref.in.long.globocan,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.eu27.asirr <- compute_age_standardized_incidence_rate_ratio(
        dat.in.long,
        dat.ref.in.long.globocan.eu27,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.euUN.asirr <- compute_age_standardized_incidence_rate_ratio(
        dat.in.long,
        dat.ref.in.long.globocan.euUN,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.cird <- compute_incidence_rates_difference(
        dat.in.long,
        dat.ref.in.long,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.cird <- compute_incidence_rates_difference(
        dat.in.long,
        dat.ref.in.long.globocan,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.eu27.cird <- compute_incidence_rates_difference(
        dat.in.long,
        dat.ref.in.long.globocan.eu27,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.euUN.cird <- compute_incidence_rates_difference(
        dat.in.long,
        dat.ref.in.long.globocan.euUN,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.asird <- compute_age_standardized_incidence_rate_difference(
        dat.in.long,
        dat.ref.in.long,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.asird <- compute_age_standardized_incidence_rate_difference(
        dat.in.long,
        dat.ref.in.long.globocan,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.eu27.asird <- compute_age_standardized_incidence_rate_difference(
        dat.in.long,
        dat.ref.in.long.globocan.eu27,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.euUN.asird <- compute_age_standardized_incidence_rate_difference(
        dat.in.long,
        dat.ref.in.long.globocan.euUN,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.sir <- compute_indirect_standardized_incidence_ratio(
        dat.in.long,
        dat.ref.in.long,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.sir <- compute_indirect_standardized_incidence_ratio(
        dat.in.long,
        dat.ref.in.long.globocan,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.eu27.sir <- compute_indirect_standardized_incidence_ratio(
        dat.in.long,
        dat.ref.in.long.globocan.eu27,
        std.pop,
        aggr.level = aggr.level,
      ...
      dat.globocan.euUN.sir <- compute_indirect_standardized_incidence_ratio(
        dat.in.long,
        dat.ref.in.long.globocan.euUN,
        std.pop,
        aggr.level = aggr.level,
        ...
        function(
          ...
          left_join(
            ...
            dat |>
              ...
              select(all_of(c(aggr.level, 'sex', 'ageg', 'ntot' = ncan.lab))),
            by = c(aggr.level, 'sex', 'ageg')
          ) |>
            left_join(
              left_join(
                dat.ref |>
                  ...
                  select(all_of(c('sex', 'ageg', 'can', ncanref.lab))),
                dat.ref |>
                  filter(
                    !(.data$ageg %in% c('total')),
                    .data$can == 'allC',
                  ...
                  select(all_of(c('sex', 'ageg', 'ntotref' = ncanref.lab))),
                by = c('sex', 'ageg')
              ),
              by = c('sex', 'ageg', 'can')
            ) |>
      ...
      if (aggr.level == 'asr_rank_cat') {
        ## special case of burden aggregation
        dat.pir <- compute_indirect_proportional_incidence_ratio_burden(
          dat.in.long.raw,
          dat.ref.in.long,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
        ...
        dat.globocan.pir <- compute_indirect_proportional_incidence_ratio_burden(
          dat.in.long.raw,
          dat.ref.in.long.globocan,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
        ...
        dat.globocan.eu27.pir <- compute_indirect_proportional_incidence_ratio_burden(
          dat.in.long.raw,
          dat.ref.in.long.globocan.eu27,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
        ...
        dat.globocan.euUN.pir <- compute_indirect_proportional_incidence_ratio_burden(
          dat.in.long.raw,
          dat.ref.in.long.globocan.euUN,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
        ...
        dat.pir <- compute_indirect_proportional_incidence_ratio(
          dat.in.long,
          dat.ref.in.long,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
        ...
        dat.globocan.pir <- compute_indirect_proportional_incidence_ratio(
          dat.in.long,
          dat.ref.in.long.globocan,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
        ...
        dat.globocan.eu27.pir <- compute_indirect_proportional_incidence_ratio(
          dat.in.long,
          dat.ref.in.long.globocan.eu27,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
        ...
        dat.globocan.euUN.pir <- compute_indirect_proportional_incidence_ratio(
          dat.in.long,
          dat.ref.in.long.globocan.euUN,
          aggr.level = aggr.level,
          ncan.lab = 'ncan',
      ...
      dat.ncan <-
        ...
        mutate(
          n_tot = .data$n_tot |>
            replace(.data$n_tot < ncan.min, paste('<', ncan.min)),
          n_mv = .data$n_mv |>
            replace(
              (.data$n_tot == paste('<', ncan.min) | is.na(.data$n_tot)),
              NA
            ),
          n_dco = .data$n_dco |>
            replace(
      ...
      dat.ref.ncan <-
        ...
        mutate(
          ...
          py = .data$py |>
            replace(.data$py < py.min, paste('<', py.min))
        )
      ...
      ir_pr_gen <-
        dat.ref.in.long |>
        rename('ncan' = 'ncanref', 'py' = 'pyref') |>
        filter(
          !(.data$ageg %in% c('total')),
          !(is.na(.data$ncan) & is.na(.data$py))
        ) |>
        left_join(
          dat.ref.ir |>
            rename('ir' = 'est', 'ir_lci' = 'lci', 'ir_uci' = 'uci'),
          by = c('sex', 'ageg', 'can')
        ) |>
        left_join(
          dat.ref.pr |>
            rename('pr' = 'est', 'pr_lci' = 'lci', 'pr_uci' = 'uci'),
          by = c('sex', 'ageg', 'can')
        ) |>
        mutate(population = 'general population', .before = 1) |>
      ...
      ir_pr_migr <-
        ...
        left_join(
          dat.ir |> rename('ir' = 'est', 'ir_lci' = 'lci', 'ir_uci' = 'uci'),
          by = c(aggr.level, 'sex', 'ageg', 'can')
        ) |>
        left_join(
          dat.pr |> rename('pr' = 'est', 'pr_lci' = 'lci', 'pr_uci' = 'uci'),
          by = c(aggr.level, 'sex', 'ageg', 'can')
        ) |>
        filter(!(.data$ageg %in% c('00_04', '05_09', '10_14', '15_19')))
      ...
      other_gen <-
        dat.ref.cir |>
        rename('cir' = 'est', 'cir_lci' = 'lci', 'cir_uci' = 'uci') |>
        full_join(
          dat.ref.asir |>
            rename('asir' = 'est', 'asir_lci' = 'lci', 'asir_uci' = 'uci'),
          by = c('sex', 'can')
        ) |>
        mutate(population = 'general population', .before = 1)
      ...
      other_migr <-
        ...
        full_join(
          dat.cirr |>
            rename('cirr' = 'est', 'cirr_lci' = 'lci', 'cirr_uci' = 'uci'),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.cird |>
            rename('cird' = 'est', 'cird_lci' = 'lci', 'cird_uci' = 'uci'),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.asir |>
        ...
        full_join(
          dat.asirr |>
            rename('asirr' = 'est', 'asirr_lci' = 'lci', 'asirr_uci' = 'uci'),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.asird |>
            rename('asird' = 'est', 'asird_lci' = 'lci', 'asird_uci' = 'uci'),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.sir |>
            rename('sir' = 'est', 'sir_lci' = 'lci', 'sir_uci' = 'uci'),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.pir |>
            rename('pir' = 'est', 'pir_lci' = 'lci', 'pir_uci' = 'uci'),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        ...
        full_join(
          dat.globocan.asirr |>
            rename(
              'asirr_glonat' = 'est',
              'asirr_glonat_lci' = 'lci',
              'asirr_glonat_uci' = 'uci'
            ),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.globocan.asird |>
            rename(
              'asird_glonat' = 'est',
              'asird_glonat_lci' = 'lci',
              'asird_glonat_uci' = 'uci'
            ),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
        ...
        full_join(
          dat.globocan.eu27.cirr |>
            rename(
              'cirr_gloeu27' = 'est',
              'cirr_gloeu27_lci' = 'lci',
              'cirr_gloeu27_uci' = 'uci'
            ),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.globocan.eu27.cird |>
            rename(
              'cird_gloeu27' = 'est',
              'cird_gloeu27_lci' = 'lci',
              'cird_gloeu27_uci' = 'uci'
            ),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.globocan.eu27.asirr |>
            rename(
              'asirr_gloeu27' = 'est',
              'asirr_gloeu27_lci' = 'lci',
              'asirr_gloeu27_uci' = 'uci'
            ),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.globocan.eu27.asird |>
            rename(
              'asird_gloeu27' = 'est',
              'asird_gloeu27_lci' = 'lci',
              'asird_gloeu27_uci' = 'uci'
            ),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.globocan.eu27.sir |>
            rename(
              'sir_gloeu27' = 'est',
              'sir_gloeu27_lci' = 'lci',
              'sir_gloeu27_uci' = 'uci'
        ...
        full_join(
          dat.globocan.euUN.cirr |>
            rename(
              'cirr_gloeuUN' = 'est',
              'cirr_gloeuUN_lci' = 'lci',
              'cirr_gloeuUN_uci' = 'uci'
            ),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.globocan.euUN.cird |>
            rename(
              'cird_gloeuUN' = 'est',
              'cird_gloeuUN_lci' = 'lci',
              'cird_gloeuUN_uci' = 'uci'
            ),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.globocan.euUN.asirr |>
            rename(
              'asirr_gloeuUN' = 'est',
              'asirr_gloeuUN_lci' = 'lci',
              'asirr_gloeuUN_uci' = 'uci'
            ),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.globocan.euUN.asird |>
            rename(
              'asird_gloeuUN' = 'est',
              'asird_gloeuUN_lci' = 'lci',
              'asird_gloeuUN_uci' = 'uci'
            ),
          by = c(aggr.level, 'sex', 'can')
        ) |>
        full_join(
          dat.globocan.euUN.sir |>
            rename(
              'sir_gloeuUN' = 'est',
              'sir_gloeuUN_lci' = 'lci',
              'sir_gloeuUN_uci' = 'uci'
    ...
    list_data_info_out <-
      list(
        data_info = data.info |>
          magrittr::set_colnames(c('DATA INFORMATION', ''))
      )
    ...
    readme.file <- system.file(
      "extdata",
      "readme_cancerradarr_output_file.xlsx",
